...
static void transform(SHA256 *this, const uint8_t *message, unsigned int block_nb) {
  uint32_t w[64];
  uint32_t wv[8];
  uint32_t t1, t2;
...
static void update(SHA256 *this, const uint8_t *message, unsigned int len) {
  unsigned int block_nb;
  unsigned int new_len, rem_len, tmp_len;
  const uint8_t *shifted_message;
  tmp_len = SHA224_256_BLOCK_SIZE - this->m_len;
  ...
  transform(this, shifted_message, block_nb);
...
static void final(SHA256 *this, uint8_t *digest) {
  unsigned int block_nb;
  unsigned int pm_len;
  unsigned int len_b;
  int i;
  block_nb = (1 + ((SHA224_256_BLOCK_SIZE - 9) < (this->m_len % SHA224_256_BLOCK_SIZE)));
  len_b = (this->m_tot_len + this->m_len) << 3;
  pm_len = block_nb << 6;
  memset(this->m_block + this->m_len, 0, pm_len - this->m_len);
  this->m_block[this->m_len] = 0x80;
  SHA2_UNPACK32(len_b, this->m_block + pm_len - 4);
  transform(this, this->m_block, block_nb);
  for (i = 0; i < 8; i++) {
