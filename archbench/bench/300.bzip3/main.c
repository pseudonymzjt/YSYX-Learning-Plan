...
static const struct {
  ...
  uint64_t ref_time;
         ...
         {"input/test-hello.txt", KiB(65), 39, 0xca9643e5, 0},
         {"input/train-queen.c", KiB(384), 564, 0x47ff882e, 560},
         {"input/ref-universe.jpg", MiB(1), 4984391, 0x8b27c89d, 886563},
         {"input/huge.model", MiB(511), 50431625, 0x5672cbf2, 6211844},
};
static Obuf obuf;
...
static void xwrite(const void *data, size_t size, size_t len, FILE *des) {
  ...
  bench_obuf_append(&obuf, data, size * len);
#if 0
...
static void close_out_file(FILE *des) {
  if (des) {
    if (fflush(des)) {
      fprintf(stderr, "Error: Failed on fflush\n");
      exit(1);
    }

    if (des != stdout && fclose(des)) {
      fprintf(stderr, "Error: Failed on fclose\n");
      exit(1);
    }
  }
}
...
static int process(FILE *input_des, FILE *output_des, int mode, int block_size, int workers, int verbose,
  ...
  if (verbose) {
    ...
    if (mode == MODE_ENCODE)
      fprintf(stderr, "\t%" PRIu64 " -> %" PRIu64 " bytes, %" PRIu64 "%%, %" PRIu64 " bpb\n", bytes_read, bytes_written,
              bytes_written * 100 / bytes_read, bytes_written * 8 / bytes_read);
    else if (mode == MODE_DECODE)
      fprintf(stderr, "\t%" PRIu64 " -> %" PRIu64 " bytes, %" PRIu64 "%%, %" PRIu64 " bpb\n", bytes_read, bytes_written,
              bytes_read * 100 / bytes_written, bytes_read * 8 / bytes_written);
    else
      fprintf(stderr, "\tOK, %" PRIu64 " -> %" PRIu64 " bytes, %" PRIu64 "%%, %" PRIu64 " bpb\n", bytes_read,
              bytes_written, bytes_read * 100 / bytes_written, bytes_read * 8 / bytes_written);
  }
...
static int mode = MODE_ENCODE;
static u32 block_size; // the block size
FILE *input_des, *output_des;
static int r;
static int workers = 0, verbose = 1, remove_input_file = 0;
static char *input; // input file name

void bench_init(int setting) {
  ps = &settings[setting];

  char *argv[] = {"./a.out", ps->data, NULL};
  int argc = sizeof(argv) / sizeof(argv[0]) - 1;

  bench_obuf_init(&obuf, 1024);
  ...
  int force_stdstreams = 0, batch = 0;

  // the block size
  block_size = ps->block_size;

  enum {
    RM_OPTION = CHAR_MAX + 1
  };

  yarg_options opt[] = {{'e', no_argument, "encode"},
...
#endif
                        {0, no_argument, NULL}};
  yarg_settings settings0 = {
    .dash_dash = true,
    .style = YARG_STYLE_UNIX,
  };
  yarg_result *res = yarg_parse(argc, argv, opt, settings0);
  if (!res) {
  ...
  for (int i = 0; i < res->argc; i++) {
    switch (res->args[i].opt) {
      ...
      case 'k': break;
      case 'h': help(); bench_exit(0);
      case 'V': version(); bench_exit(0);
...
uint64_t bench_run() {
  r = process(input_des, output_des, mode, block_size, workers, verbose, input);
  return ps->ref_time;
}
...
int bench_validate() {
  ...
  if (obuf.len != ps->len || (cs != ps->checksum)) {
    r = 1;
  }
