...
static int yarg_parse_unix(int argc, char *argv[], yarg_options opt[], yarg_result *res, bool dash_dash) {
  ...
  for (int i = 1; i < argc; i++) {
    if (argv[i][0] == '-') {
      if (argv[i][1] == '-') {
        ...
        yarg_options *o = NULL;
        int len = 0;
        while (long_opt[len] && long_opt[len] != '=')
          len++;
...
static int yarg_parse_unix_short(int argc, char *argv[], yarg_options opt[], yarg_result *res, bool dash_dash,
                                 char opt_char) {
  int no_args = 0, no_pos_args = 0;
  for (int i = 1; i < argc; i++) {
...
yarg_result *yarg_parse(int argc, char *argv[], yarg_options opt[], yarg_settings settings) {
  ...
  switch (settings.style) {
    case YARG_STYLE_WINDOWS: yarg_parse_unix_short(argc, argv, opt, res, false, '/'); break;
    case YARG_STYLE_UNIX: yarg_parse_unix(argc, argv, opt, res, settings.dash_dash); break;
    case YARG_STYLE_UNIX_SHORT: yarg_parse_unix_short(argc, argv, opt, res, settings.dash_dash, '-'); break;
  }
  return res;
}
