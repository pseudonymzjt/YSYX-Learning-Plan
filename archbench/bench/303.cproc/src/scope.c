...
void scopeinit(void) {
  ...
  memset(&filescope, 0, sizeof(filescope));
...
struct scope *mkscope(struct scope *parent) {
  ...
  s->parent = parent;
  s->func = parent ? parent->func : NULL;

  return s;
}

struct scope *delscope(struct scope *s) {
...
struct decl *scopegetdecl(struct scope *s, const char *name, bool recurse) {
  struct decl *d;
  struct mapkey k;
...
struct type *scopegettag(struct scope *s, const char *name, bool recurse) {
  struct type *t;
  struct mapkey k;
...
void scopeputdecl(struct scope *s, struct decl *d) {
  struct mapkey k;
...
void scopeputtag(struct scope *s, const char *name, struct type *t) {
  struct mapkey k;
