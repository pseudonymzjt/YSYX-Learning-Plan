...
struct init *mkinit(unsigned long long start, unsigned long long end, struct bitfield bits, struct expr *expr) {
  struct init *init;
...
static void initadd(struct initparser *p, struct init *new) {
...
static void subobj(struct initparser *p, struct type *t, unsigned long long off) {
  off += p->sub->offset;
...
static bool findmember(struct initparser *p, char *name) {
  struct member *m;
...
static void designator(struct scope *s, struct initparser *p) {
  struct type *t;
  char *name;
  ...
  for (;;) {
    ...
    switch (tok.kind) {
      ...
      case TPERIOD:
        ...
        free(name);
        break;
      default: expect(TASSIGN, "after designator"); return;
    }
  }
}

static void focus(struct initparser *p) {
  struct type *t;
  ...
  switch (p->sub->type->kind) {
    ...
    case TYPEUNION:
      ...
      t = p->sub->u.mem->type;
      break;
    default: fatal("internal error: init cursor has unexpected type"); return; /* unreachable */
  }
  subobj(p, t, 0);
}

static void advance(struct initparser *p) {
  struct type *t;
...
struct init *parseinit(struct scope *s, struct type *t) {
  struct initparser p;
  struct expr *expr;
  struct type *base;
  ...
  add:
    ...
    next:
      ...
      p.sub = p.cur;
      do
        p.cur = p.cur == p.obj ? NULL : p.cur - 1;
