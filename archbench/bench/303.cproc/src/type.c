...
struct type *mkpointertype(struct type *base, enum typequal qual) {
  struct type *t;
...
struct type *mkarraytype(struct type *base, enum typequal qual, unsigned long long len) {
  struct type *t;
...
static int typerank(struct type *t) {
  if (t->kind == TYPEENUM)
    t = t->base;
...
bool typecompatible(struct type *t1, struct type *t2) {
  struct decl *p1, *p2;
  struct expr *e1, *e2;
  ...
  if (t1->kind != t2->kind) {
    /*
		...
		same type)
		*/
    return t1->kind == TYPEENUM && t2 == t1->base || t2->kind == TYPEENUM && t1 == t2->base;
  }
  switch (t1->kind) {
    case TYPEPOINTER: goto derived;
    case TYPEARRAY:
...
bool typesame(struct type *t1, struct type *t2) {
  /* XXX: implement */
  return typecompatible(t1, t2);
}

struct type *typecomposite(struct type *t1, struct type *t2) {
  /* XXX: implement 6.2.7 */
  /* XXX: merge with typecompatible? */
  return t1;
}

struct type *typepromote(struct type *t, unsigned width) {
...
struct type *typecommonreal(struct type *t1, unsigned w1, struct type *t2, unsigned w2) {
  struct type *tmp;
...
struct type *typeadjust(struct type *t, enum typequal *tq) {
...
struct member *typemember(struct type *t, const char *name, unsigned long long *offset) {
  struct member *m, *sub;
...
bool typehasint(struct type *t, unsigned long long i, bool sign) {
  assert(t->prop & PROPINT);
  if (sign && i >= -1ull << 63)
