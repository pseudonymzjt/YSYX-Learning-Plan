...
void mapkey(struct mapkey *k, const void *s, size_t n) {
  k->str = s;
  k->len = n;
  k->hash = hash(s, n);
}

void mapinit(struct map *h, size_t cap) {
  size_t i;
...
void mapfree(struct map *h, void del(void *)) {
  size_t i;
...
static bool keyequal(struct mapkey *k1, struct mapkey *k2) {
  if (k1->hash != k2->hash || k1->len != k2->len)
    return false;
  return memcmp(k1->str, k2->str, k1->len) == 0;
}

static size_t keyindex(struct map *h, struct mapkey *k) {
  size_t i;
...
void **mapput(struct map *h, struct mapkey *k) {
...
void *mapget(struct map *h, struct mapkey *k) {
  size_t i;
