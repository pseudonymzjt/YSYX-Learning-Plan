...
static void cast(struct expr *expr) {
  unsigned size;
  unsigned long long m;

  size = expr->type->size;
  if (expr->type->prop & PROPINT) {
...
static void unary(struct expr *expr, enum tokenkind op, struct expr *l) {
  expr->kind = EXPRCONST;
  switch (op) {
    case TSUB: expr->u.constant.u = -l->u.constant.u; break;
    default: fatal("internal error; unknown unary expression");
  }
  cast(expr);
}

static void binary(struct expr *expr, enum tokenkind op, struct expr *l, struct expr *r) {
  expr->kind = EXPRCONST;
  if (l->type->prop & PROPINT && l->type->u.basic.issigned)
    op |= S;
  switch (op) {
    case TMUL:
    ...
    default: fatal("internal error; unknown binary expression");
  }
  cast(expr);
}
    ...
    case EXPRBINARY:
      ...
      switch (expr->op) {
        ...
        case TSUB:
          ...
          } else if (l->kind == EXPRBINARY && l->type->kind == TYPEPOINTER && l->op == TADD &&
                     l->u.binary.r->kind == EXPRCONST) {
            /* (P + C1) ± C2  ->  P + (C1 ± C2) */
