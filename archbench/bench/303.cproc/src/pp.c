...
static bool macroequal(struct macro *m1, struct macro *m2) {
...
static size_t macroparam(struct macro *m, struct token *t) {
  size_t i;
...
static struct macro *macroget(char *name) {
  struct mapkey k;
...
static void macrodone(struct macro *m) {
  m->hide = false;
...
static bool macrovarargs(struct macro *m) {
  return m->kind == MACROFUNC && m->nparam > 0 && m->param[m->nparam - 1].flags & PARAMVAR;
}

static struct token *framenext(struct frame *f) {
  return f->ntoken--, f->token++;
}

/* push a new context frame */
static struct frame *ctxpush(struct token *t, size_t n, struct macro *m, bool space) {
  struct frame *f;
...
static struct token *ctxnext(void) {
  struct frame *f;
  struct token *t;
  struct macro *m;
  bool space;
...
static void nextinto(struct token *t) {
  for (;;) {
    scan(t);
...
static struct token *rawnext(void) {
  struct token *t;
...
static bool peekparen(void) {
  ...
  pending.len = 0;
  do
    t = arrayadd(&pending, sizeof(*t)), nextinto(t);
...
static void stringize(struct array *buf, struct token *t) {
  const char *lit;
...
static bool expand(struct token *t) {
  struct macro *m;
  bool space;
...
static void expandfunc(struct macro *m) {
  struct macroparam *p;
  struct macroarg *arg;
...
static void keyword(struct token *tok, bool dofree) {
  static const struct {
    const char *name;
    int value;
  } keywords[] = {
...
char *expect(enum tokenkind kind, const char *msg) {
  char *lit;
