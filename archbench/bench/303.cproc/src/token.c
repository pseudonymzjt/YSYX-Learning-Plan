...
void tokenprint(const struct token *t) {
  const char *str;
  ...
  switch (t->kind) {
    ...
    case TNUMBER:
    case TCHARCONST:
    case TSTRINGLIT: str = t->lit; break;
    case TNEWLINE: str = "\n"; break;
    case TEOF: return;
    default: str = tokstr[t->kind];
  }
  if (!str)
...
static void tokendesc(char *buf, size_t len, enum tokenkind kind, const char *lit) {
  const char *class;
  bool quote = true;
  ...
  switch (kind) {
    ...
    default: class = NULL; lit = kind < LEN(tokstr) ? tokstr[kind] : NULL;
  }
  if (class && lit)
...
char *tokencheck(const struct token *t, enum tokenkind kind, const char *msg) {
  char want[64], got[64];
