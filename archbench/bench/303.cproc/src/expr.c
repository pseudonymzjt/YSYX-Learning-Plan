...
static struct expr *mkexpr(enum exprkind k, struct type *t, struct expr *b) {
  struct expr *e;
...
void delexpr(struct expr *e) {
  ...
  switch (e->kind) {
    ...
    case EXPRINCDEC:
    case EXPRUNARY:
    case EXPRCAST: delexpr(e->base); break;
    case EXPRBINARY:
...
static struct expr *mkconstexpr(struct type *t, unsigned long long n) {
  struct expr *e;
...
static struct expr *decay(struct expr *e) {
  struct type *t;
  enum typequal tq;
...
static struct expr *mkunaryexpr(enum tokenkind op, struct expr *base) {
  struct expr *expr;
  struct type *type;
  ...
  switch (op) {
    case TBAND:
      ...
      /*
		...
		*/
      if (!base->lvalue && base->type->kind != TYPEFUNC && base->type->kind != TYPESTRUCT &&
          base->type->kind != TYPEUNION)
...
struct expr *exprassign(struct expr *e, struct type *t) {
  struct type *et;
...
struct expr *exprpromote(struct expr *e) {
  struct type *t;
...
static struct type *commonreal(struct expr **e1, struct expr **e2) {
  struct type *t;
...
static struct expr *mkbinaryexpr(struct location *loc, enum tokenkind op, struct expr *l, struct expr *r) {
  struct expr *e;
  struct type *t = NULL;
  enum typeprop lp, rp;
  ...
  switch (op) {
    ...
    case TBOR:
    case TXOR:
    case TBAND: t = commonreal(&l, &r); break;
    case TADD:
      if (lp & PROPARITH && rp & PROPARITH) {
        t = commonreal(&l, &r);
        break;
    ...
    case TSHR:
      ...
      t = l->type;
      break;
    default: fatal("internal error: unknown binary operator %d", op);
  }
...
static struct type *inttype(unsigned long long val, bool decimal, char *end) {
  static struct {
    struct type *type;
    const char *end1, *end2;
  } limits[] = {
    {&typeint, "", NULL},     {&typeuint, "u", NULL},   {&typelong, "l", NULL},
    {&typeulong, "ul", "lu"}, {&typellong, "ll", NULL}, {&typeullong, "ull", "llu"},
  };
  struct type *t;
  size_t i, step;
...
static size_t decodechar(const char *src, uint32_t *chr, bool *hexoct, const char *desc, struct location *loc) {
  uint32_t c;
  size_t n;
  int i;
...
static size_t encodechar8(void *dst, uint32_t chr, bool hexoct) {
  if (!hexoct)
...
static size_t encodechar16(void *dst, uint32_t chr, bool hexoct) {
  if (!hexoct)
    return utf16enc(dst, chr) * sizeof(uint16_t);
  *(uint16_t *)dst = chr;
  return sizeof(uint16_t);
}

static size_t encodechar32(void *dst, uint32_t chr, bool hexoct) {
  *(uint32_t *)dst = chr;
  return sizeof(uint32_t);
}

struct type *stringconcat(struct stringlit *str, bool forceutf8) {
  ...
  int kind, newkind;
  struct type *t;
  size_t (*encodechar)(void *, uint32_t, bool);
  char *src;
...
static struct expr *generic(struct scope *s) {
...
/* 6.5 Expressions */
static struct expr *primaryexpr(struct scope *s) {
  struct expr *e;
  struct decl *d;
  struct type *t;
  char *src, *end;
  ...
  switch (tok.kind) {
    ...
    case TCHARCONST:
      ...
      switch (*src) {
        ...
        case 'u':
          ++src;
          t = *src == '8' ? ++src, &typeuchar : &typeushort;
          break;
        case 'U':
          ++src;
          t = &typeuint;
    ...
    case TNUMBER:
      ...
      if (strpbrk(tok.lit, base == 16 ? ".pP" : ".eE")) {
        /* floating constant */
        error(&tok.loc, "floating constants are not supported");
      } else {
        src = tok.lit;
        if (base == 2)
          src += 2;
    ...
    case TLPAREN:
      ...
      break;
    case T_GENERIC: e = generic(s); break;
    default: error(&tok.loc, "expected primary expression"); return NULL; /* unreachable */
  }
...
static void designator(struct scope *s, struct type *t, unsigned long long *offset) {
  char *name;
  struct member *m;
...
static struct expr *builtinfunc(struct scope *s, enum builtinkind kind) {
  ...
  switch (kind) {
    case BUILTINALLOCA:
      ...
      break;
    case BUILTINCONSTANTP: e = mkconstexpr(&typeint, eval(condexpr(s))->kind == EXPRCONST); break;
    case BUILTINEXPECT:
    ...
    case BUILTINVASTART:
      ...
      if (consume(TCOMMA))
        delexpr(assignexpr(s));
      break;
    default: fatal("internal error; unknown builtin"); return NULL; /* unreachable */
  }
  return e;
}

static struct expr *mkincdecexpr(enum tokenkind op, struct expr *base, bool post) {
  struct expr *e;
...
static struct expr *postfixexpr(struct scope *s, struct expr *r) {
...
static struct expr *unaryexpr(struct scope *s) {
  enum tokenkind op;
  struct expr *e, *l;
  struct type *t;
  ...
  switch (op) {
    ...
    case TDEC:
      ...
      break;
    case TBAND:
    case TMUL: next(); return mkunaryexpr(op, castexpr(s));
    case TADD:
      next();
      e = castexpr(s);
    ...
    case TALIGNOF:
      ...
      break;
    default: e = postfixexpr(s, NULL);
  }

  return e;
}

static struct expr *castexpr(struct scope *s) {
  struct type *t, *ct;
  struct decl *d;
  enum typequal tq;
  ...
  while (consume(TLPAREN)) {
    ...
    if (!t) {
      if (tok.kind == TLBRACE) {
        ...
        if (!s->func)
          error(&tok.loc, "statement expression outside of function");
        v = stmt_expr(s->func, s, &rt);
        expect(TRPAREN, "after statement expression");
        e = mkexpr(EXPRTEMP, rt, NULL);
        e->u.temp = v;
        e = postfixexpr(s, e);
        goto done;
      }
...
static int precedence(enum tokenkind t) {
  switch (t) {
    case TLOR: return 0;
    case TLAND: return 1;
    case TBOR: return 2;
...
static struct expr *binaryexpr(struct scope *s, struct expr *l, int i) {
  struct expr *r;
  struct location loc;
  enum tokenkind op;
  int j, k;
...
unsigned long long intconstexpr(struct scope *s, bool allowneg) {
  struct expr *e;
...
static struct expr *mkassignexpr(struct expr *l, struct expr *r) {
  struct expr *e;
...
struct expr *assignexpr(struct scope *s) {
...
struct expr *expr(struct scope *s) {
  struct expr *r, *e, **end;
