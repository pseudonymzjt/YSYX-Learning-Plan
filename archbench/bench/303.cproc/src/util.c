...
void *arrayadd(struct array *a, size_t n) {
  void *v;
...
void arrayaddptr(struct array *a, void *v) {
  *(void **)arrayadd(a, sizeof(v)) = v;
}

void arrayaddbuf(struct array *a, const void *src, size_t n) {
  memcpy(arrayadd(a, n), src, n);
}

void *arraylast(struct array *a, size_t n) {
  if (a->len == 0)
    return NULL;
  assert(n <= a->len);
...
void listinsert(struct list *list, struct list *new) {
  new->next = list->next;
  new->prev = list;
  list->next->prev = new;
  list->next = new;
