...
struct decl *mkdecl(char *name, enum declkind k, struct type *t, enum typequal tq, enum linkage linkage) {
  struct decl *d;
...
static int storageclass(enum storageclass *sc) {
...
static int typequal(enum typequal *tq) {
  switch (tok.kind) {
...
static int funcspec(enum funcspec *fs) {
  enum funcspec new;
...
static struct type *tagspec(struct scope *s) {
...
/* 6.7 Declarations */
static struct qualtype declspecs(struct scope *s, enum storageclass *sc, enum funcspec *fs, int *align) {
  struct type *t, *other;
  struct decl *d;
  struct expr *e;
  ...
  for (;;) {
    ...
    switch (op) {
      ...
      case TBOOL:
        ...
        next();
        break;
      case T_COMPLEX: error(&tok.loc, "_Complex is not yet supported"); break;
      case T_ATOMIC: error(&tok.loc, "_Atomic is not yet supported"); break;
      case TSTRUCT:
      case TUNION:
      case TENUM:
        t = tagspec(s);
      ...
      case T__ATTRIBUTE__: gnuattr(NULL, 0); break;
  ...
  switch ((int)ts) {
    ...
    default: error(&tok.loc, "invalid combination of type specifiers");
  }
  if (!t && (tq || sc && *sc || fs && *fs))
...
static bool istypename(struct scope *s, const char *name) {
  struct decl *d;
...
declarator().
*/
static void declaratortypes(struct scope *s, struct list *result, char **name, struct scope **funcscope,
                            bool allowabstract) {
        ...
        switch (tok.kind) {
          ...
          case TIDENT:
            if (!istypename(s, tok.lit))
              break;
            /* fallthrough */
          default: allowattr = true; goto func;
        }
      }
...
static struct qualtype declarator(struct scope *s, struct qualtype base, char **name, struct scope **funcscope,
                                  bool allowabstract) {
  struct type *t;
  enum typequal tq;
  struct expr *e;
...
static struct decl *parameter(struct scope *s) {
  struct decl *d;
  char *name;
  struct qualtype t;
...
static void addmember(struct structbuilder *b, struct qualtype mt, char *name, int align, unsigned long long width) {
...
static bool staticassert(struct scope *s) {
...
static void structdecl(struct scope *s, struct structbuilder *b) {
...
/* 6.7.7 Type names */
struct type *typename(struct scope *s, enum typequal *tq, struct expr **toeval) {
  struct qualtype t;
...
static enum linkage getlinkage(enum declkind kind, enum storageclass sc, struct decl *prior, bool filescope) {
  if (sc & SCSTATIC)
...
static struct decl *declcommon(struct scope *s, enum declkind kind, char *name, char *asmname, struct type *t,
                               enum typequal tq, enum storageclass sc, struct decl *prior) {
  struct decl *d;
...
static void defineobj(struct decl *d, struct init *init, bool hasinit, struct func *f) {
...
bool decl(struct scope *s, struct func *f) {
    ...
    switch (kind) {
      ...
      case DECLOBJECT:
        if (align && align < t->align)
          error(&tok.loc, "object '%s' requires alignment %d, which is stricter than specified alignment %d", name,
                t->align, align);
        ...
        if (d->linkage == LINKNONE && !(sc & SCSTATIC)) {
          ...
          if (t->prop & PROPVM)
            error(&tok.loc, "object '%s' with %s storage duration cannot have variably modified type", name,
                  d->u.obj.storage == SDSTATIC ? "static" : "thread");
          d->value = mkglobal(d);
        }
        ...
        if (consume(TASSIGN)) {
          if (f && d->linkage != LINKNONE)
            error(&tok.loc, "object '%s' with block scope and %s linkage cannot have initializer", name,
                  d->linkage == LINKEXTERN ? "external" : "internal");
          if (d->defined)
      ...
      case DECLFUNC:
        ...
        d->value = mkglobal(d);
        d->u.func.inlinedefn =
            d->linkage == LINKEXTERN && fs & FUNCINLINE && !(sc & SCEXTERN) && (!prior || prior->u.func.inlinedefn);
...
void decl_init() {
  tentativedefns = NULL;
  tentativedefnsend = &tentativedefns;
