// ported from https://github.com/pantaloons/RSA/blob/0055a474bf0d11a52f2232813fedfaa4173cb5a1/multiple.c
...
#define NOPRINT
#define DEF_STATIC_BIGNUM(name) \
  static bignum name##_obj;     \
  bignum *name = &name##_obj
...
static const struct {
  ...
  uint64_t ref_time;
         ...
         {"input/test.txt", 2, 0},
         {"input/train.txt", 6, 217},
         {"input/ref.txt", 100, 516106},
         {"input/ref.txt", 512, 118110931},
};
...
word DATA9[1] = {9};
word DATA10[1] = {10};
bignum NUMS[11] = {{1, 1, DATA0}, {1, 1, DATA1}, {1, 1, DATA2}, {1, 1, DATA3}, {1, 1, DATA4}, {1, 1, DATA5},
                   {1, 1, DATA6}, {1, 1, DATA7}, {1, 1, DATA8}, {1, 1, DATA9}, {1, 1, DATA10}};
...
void bignum_print(bignum *b) {
  ...
  DEF_STATIC_BIGNUM(copy);
  DEF_STATIC_BIGNUM(remainder);
...
void bignum_subtract(bignum *result, bignum *b1, bignum *b2) {
  ...
  for (i = 0; i < b1->length; i++) {
    ...
    if (temp > b1->data[i])
      ...
      if (temp == 0 && b2->data != NULL && b2->data[i] == 0xffffffff) {
        carry = 1;
      } else {
        carry = 0;
      }
...
void bignum_multiply(bignum *result, bignum *b1, bignum *b2) {
  ...
  unsigned long long int
      prod; /* Long for intermediate product... this is not portable and should probably be changed */
 ...
 */
void bignum_divide(bignum *quotient, bignum *remainder, bignum *b1, bignum *b2) {
  DEF_STATIC_BIGNUM(b2copy);
  DEF_STATIC_BIGNUM(b1copy);
  DEF_STATIC_BIGNUM(temp);
  DEF_STATIC_BIGNUM(temp2);
  DEF_STATIC_BIGNUM(temp3);
  DEF_STATIC_BIGNUM(quottemp);
  if (quottemp->data == NULL) {
    bignum_copy(&NUMS[0], quottemp);
  }
  ...
  if (bignum_less(b1, b2)) { /* Trivial case, b1/b2 = 0 iff b1 < b2. */
    ...
    for (i = n - m - 1; i >= 0; i--) {
      ...
      while (grem < RADIX &&
             gquot * b2copy->data[m - 2] > RADIX * grem + b1copy->data[i + m - 2]) { /* Should not overflow... ? */
        gquot--;
...
void bignum_inverse(bignum *a, bignum *m, bignum *result) {
  DEF_STATIC_BIGNUM(remprev);
  DEF_STATIC_BIGNUM(rem);
  DEF_STATIC_BIGNUM(auxprev);
  DEF_STATIC_BIGNUM(aux);
  DEF_STATIC_BIGNUM(rcur);
  DEF_STATIC_BIGNUM(qcur);
  DEF_STATIC_BIGNUM(acur);
...
int solovayPrime(int a, bignum *n) {
  DEF_STATIC_BIGNUM(ab);
  DEF_STATIC_BIGNUM(res);
  DEF_STATIC_BIGNUM(pow);
  DEF_STATIC_BIGNUM(modpow);
  int x, result;
...
int probablePrime(bignum *n, int k) {
  ...
  while (k-- > 0) {
    if (n->length <= 1) { /* Prevent a > n */
      if (!solovayPrime(bench_rand() % (n->data[0] - 2) + 2, n))
        return 0;
    } else {
      int wit = bench_rand() % (RAND_MAX - 2) + 2;
...
void randPrime(int numDigits, bignum *result) {
  ...
  int i;
  string[0] = (bench_rand() % 9) + '1';                 /* No leading zeros */
  string[numDigits - 1] = (bench_rand() % 5) * 2 + '1'; /* Last digit is odd */
  for (i = 1; i < numDigits - 1; i++)
    string[i] = (bench_rand() % 10) + '0';
  string[numDigits] = '\0';
...
static bignum *p, *q, *n;
static bignum *phi, *e, *d;
static bignum *bbytes, *shift;
static bignum *temp1, *temp2;
static bignum *encoded;
static char *decoded;
static char *buffer;
static int bytes;
static int len;
...
uint64_t bench_run() {
  randPrime(ps->digits, p);
  printf("Got first prime factor, p = ");
  bignum_print(p);
  printf("\n");

  randPrime(ps->digits, q);
  ...
  while (bignum_less(bbytes, n)) {
    bignum_imultiply(
        bbytes, shift); /* Shift by one byte, NB: we use bitmask representative so this can actually be a shift... */
    bytes++;
  }

  printf("Opening file \"%s\" for reading\n", ps->text);
  FILE *f = fopen(ps->text, "r");
  if (f == NULL) {
  ...
  printf("File \"%s\" read successfully, %d bytes read. Encoding byte stream in chunks of %d bytes\n", ps->text, len,
         bytes);
...
int bench_validate() {
  int same = (memcmp(buffer, decoded, len) == 0);
  if (same == 0) {
    for (int i = 0; i < len; i++) {
      if (buffer[i] != decoded[i]) {
        printf("bytes %d different: expect %02x, get %02x\n", i, buffer[i], decoded[i]);
      }
    }
  }
