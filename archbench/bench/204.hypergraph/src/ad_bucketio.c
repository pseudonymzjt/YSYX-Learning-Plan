...
/* create a partb node */
void create_partb_node(int noparts, int mov_cell_no, int home_part, int mapped_dest_part, int mov_gain_inx,
                       partb_t **partb, cells_info_t cells_info[]) {
  ...
  insert_partb_node(tnode_ptr, mapped_dest_part, mov_gain_inx, &partb[home_part][mapped_dest_part],
                    &cells_info[mov_cell_no]);
} /* create_partb_node */

/* insert a node into partb */
void insert_partb_node(bnode_ptr_t tnode_ptr, int mapped_dest_part, int mov_gain_inx, partb_t *partb_ptr,
...
void delete_partb_nodes_of_cell(int noparts, int mov_cell_no, int home_part, partb_t **partb,
                                cells_info_t cells_info[]) {
  for (int mapped_dest_part = 0; mapped_dest_part < (noparts - 1); mapped_dest_part++) {
    delete_partb_node(True, mapped_dest_part, &partb[home_part][mapped_dest_part], &cells_info[mov_cell_no]);
  }
} /* delete_partb_nodes_of_cell */

/* delete a partb node */
bnode_ptr_t delete_partb_node(int deletion_ok, int mapped_dest_part, partb_t *partb_ptr, cells_info_t *cells_info_ptr) {
  /*
      cells_info_ptr = cells_info[mov_cell_no]
      partb_ptr = partb[home_part][mapped_dest_part]
    */
    ...
    if (tnode_ptr->lptr == NULL) {   /* if 1st node */
      ...
      if (partb_ptr->bnode_ptr[mov_gain_inx] == NULL) {
        ...
        if (partb_ptr->nobuckets == 0) { /* if no more full buckets */
          ...
          while ((partb_ptr->max_inx > partb_ptr->min_inx) && (partb_ptr->bnode_ptr[partb_ptr->max_inx] == NULL)) {
            partb_ptr->max_inx--;
          } /* while */
        } /* else */
      } /* if bucket is empty */
