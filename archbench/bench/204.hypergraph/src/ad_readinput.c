...
void init_nets(int nonets, int noparts, nets_t nets[]) {
...
void init_netlist(int nonets, cells_t cells[], nets_t nets[], corn_t cnets[], corn_t ncells[]) {
  for (int i = 0; i < nonets; i++) {
...
void read_hgraph_size(char fname[], int *nocells, int *nonets, int *nopins) {
  FILE *fp;
...
void read_hgraph(char fname[], int nocells, int nonets, int nopins, int noparts, int *totcellsize, int *totnetsize,
                 int *max_cdeg, int *max_ndeg, int *max_cweight, int *max_nweight, cells_t cells[], nets_t nets[],
                 corn_t cnets[], corn_t ncells[]) {
  FILE *fp;
  open_file(&fp, fname, "r");
  ...
  *totnetsize = 0;
  ...
  for (int i = 0; i < nonets; i++) {
    if (fscanf(fp, "%d%d", &nets[i].nweight, &nets[i].nno_cells) == EOF) {
      printf("Error: Cannot read from %sn", fname);
      close_file(&fp);
      bench_exit(1);
    }
