...
int create_partition(int nocells, int noparts, int totcellsize, int max_cweight, int *off_ratio_x20, cells_t cells[],
                     nets_t nets[], corn_t cnets[], ind_t *ind) {
  /* init current size */
  for (int i = 0; i < noparts; i++) {
    ind->parts[i].pratio_inv = noparts;
  ...
  int part_fit = True;
  int max_size = -1;
  while (((*off_ratio_x20) < 21) && (part_fit)) {

    for (int i = 0; i < noparts; i++) {
      if (totcellsize < max_cweight * ind->parts[i].pratio_inv) {
        printf("\nError: Too small part size.\n");
        bench_exit(1);
      } /* if */
      ind->parts[i].pmax_size = (totcellsize + totcellsize * *off_ratio_x20 / 20) / ind->parts[i].pratio_inv + 1;
      ind->parts[i].pmin_size = (totcellsize - totcellsize * *off_ratio_x20 / 20) / ind->parts[i].pratio_inv - 1;
...
void copy_partition(int noparts, parts_info_t parts_info[], ind_t *ind) {
  for (int i = 0; i < noparts; i++) {
...
int read_partition(FILE *fp, char *filename, int noparts, cells_t cells[], nets_t nets[], corn_t cnets[], ind_t *ind) {
  int max_size = -1;
  ...
  for (int i = 0; i < noparts; i++) {
    ...
    if (fscanf(fp, "%d%d%d%d", &(ind->parts[part_no].pmax_cells), &(ind->parts[part_no].pmin_size),
               &(ind->parts[part_no].pcurr_size), &(ind->parts[part_no].pmax_size)) == EOF) {
      printf("Error: Cannot read from %s: errno= %d error= %s\n", filename, errno, strerror(errno));
    }
...
/* write a partition */
void write_partition(FILE *fp, char *filename, int nocells, int noparts, ind_t *ind) {
  open_file(&fp, filename, "w");

  for (int i = 0; i < noparts; i++) {
    fprintf(fp, "%d %d %d %d %d\n", i, ind->parts[i].pmax_cells, ind->parts[i].pmin_size, ind->parts[i].pcurr_size,
