...
void init_buckets(int noparts, int bucketsize, partb_t **partb) {
...
int calculate_gain(int cell_no, int home_part, int dest_part, cells_info_t cells_info[]) {
  int mov_gain = cells_info[cell_no].mgain[dest_part] - cells_info[cell_no].mgain[home_part];
  return mov_gain;
} /* calculate_gain */

/* compute gains of all cells and place them into cells_info */
void compute_gains(int nocells, int noparts, cells_t cells[], nets_t nets[], corn_t cnets[], cells_info_t cells_info[],
                   allele tchrom[]) {
    ...
    for (int j = 0; j < cells[cell_no].cno_nets; j++) {
      ...
      if (nets[net_no].npartdeg[part_no] == 1) {
        ...
        while ((tpart_no < noparts) && (!found)) {
          if ((tpart_no != part_no) && (nets[net_no].npartdeg[tpart_no] == (nets[net_no].nno_cells - 1))) {
            found = True;
...
/* compute gains of all cells and place them into cells_info */
void compute_gains2(int nocells, int noparts, cells_t cells[], nets_t nets[], corn_t cnets[], cells_info_t cells_info[],
                    allele tchrom[], nets_info_t nets_info[]) {
  for (int cell_no = 0; cell_no < nocells; cell_no++) {
    ...
    for (int j = 0; j < cells[cell_no].cno_nets; j++) {
      ...
      if (nets_info[net_no].npartdeg[part_no] == 1) {
        ...
        int tpart_no = 0;
        while ((tpart_no < noparts) && (!found)) {
          if ((tpart_no != part_no) && (nets_info[net_no].npartdeg[tpart_no] == (nets[net_no].nno_cells - 1))) {
            found = True;
            cells_info[cell_no].mgain[tpart_no] += net_weight;
          } /* if tpart_no */
          tpart_no++;
...
void free_nodes(int noparts, int bucketsize, partb_t **partb) {
...
void number_nodes(int noparts, int bucketsize, int *npartb, partb_t **partb) {
  *npartb = 0;
...
int find_move_set(mcells_t mcells[], int msize, int *max_mcells_inx) {
  int max_gain_sum = 0;
  *max_mcells_inx = -1;
  int gain_sum = 0;
...
int move_cells(int wflag, int nocells, int msize, mcells_t mcells[], int max_mcells_inx, int cutsize, int *glob_inx,
               ind_t *ind, cells_t cells[], nets_t nets[], corn_t cnets[]) {
...
int find_cut_size(int nonets, int noparts, int totnetsize, nets_t nets[], ind_t *ind) {
  ind->incost = 0;
...
void copy_nets_info(int nonets, int noparts, nets_t nets[], nets_info_t nets_info[]) {
  for (int i = 0; i < nonets; i++) {
    for (int j = 0; j < noparts; j++) {
