...
void create_buckets(int nocells, int noparts, int max_gain, allele chrom[], partb_t **partb,
  ...
  for (int cell_no = 0; cell_no < nocells; cell_no++) {
    ...
    for (int mov_part_no = 0; mov_part_no < noparts; mov_part_no++) {
      if (mov_part_no != part_no) {
        ...
        /* create a partb node */
        create_partb_node(noparts, cell_no, part_no, mapped_part_no, gain_inx, partb, cells_info);
...
int select_cell(int noparts, selected_cell_t scell[], parts_info_t parts_info[], cells_t cells[], partb_t **partb,
                cells_info_t cells_info[]) {
  /*
      ...
      for (int to = 0; to < noparts; to++) {
        ...
        if ((from != to) && (tpcurr_size < tpmax_size)) {
          ...
          if (max_inx > 0) {
            ...
            if ((max_mov_value < max_inx) && (fpcurr_size >= (fpmin_size + cells[cell_no].cweight)) &&
  ...
  if (max_mov_value != -1) {
    ...
    scell[0].mov_gain = calculate_gain(scell[0].mov_cell_no, scell[0].from_part, scell[0].to_part, cells_info);
    parts_info[scell[0].from_part].pmax_cells--;
    parts_info[scell[0].to_part].pmax_cells++;
    parts_info[scell[0].from_part].pcurr_size -= cells[scell[0].mov_cell_no].cweight;
    parts_info[scell[0].to_part].pcurr_size += cells[scell[0].mov_cell_no].cweight;
...
void move_cell(mcells_t mcells[], int msize, selected_cell_t scell[]) {
...
void update_gains(int noparts, int max_gain, selected_cell_t scell[], cells_t cells[], nets_t nets[], corn_t cnets[],
                  corn_t ncells[], nets_info_t nets_info[], partb_t **partb, cells_info_t cells_info[],
                  allele tchrom[]) {
  ...
  for (int i = 0; i < cells[mov_cell_no].cno_nets; i++) {
    ...
    if (nets_info[net_no].npartdeg[from_part] == nets[net_no].nno_cells) {
      ...
      update1(False, noparts, max_gain, from_part, mov_cell_no, cell_ptr, net_no, net_weight, nets, ncells, partb,
              cells_info, tchrom);

    } else if (nets_info[net_no].npartdeg[from_part] == (nets[net_no].nno_cells - 1)) {

      update2(False, noparts, max_gain, from_part, mov_cell_no, cell_ptr, net_no, net_weight, nets, ncells, partb,
              cells_info, tchrom);
    }
    ...
    if (nets_info[net_no].npartdeg[to_part] == nets[net_no].nno_cells) {
      ...
      update1(True, noparts, max_gain, to_part, mov_cell_no, cell_ptr, net_no, net_weight, nets, ncells, partb,
              cells_info, tchrom);

    } else if (nets_info[net_no].npartdeg[to_part] == (nets[net_no].nno_cells - 1)) {

      update2(True, noparts, max_gain, to_part, mov_cell_no, cell_ptr, net_no, net_weight, nets, ncells, partb,
              cells_info, tchrom);
...
void update1(int flag, int noparts, int max_gain, int dest_part, int mov_cell_no, int cell_ptr, int net_no,
             int net_weight, nets_t nets[], corn_t ncells[], partb_t **partb, cells_info_t cells_info[],
             allele tchrom[]) {
  int i = 0;
  while (i < nets[net_no].nno_cells) {
    ...
    if ((other_cell != mov_cell_no) && (cells_info[other_cell].locked == False)) {
      ...
      delete_partb_nodes_of_cell(noparts, other_cell, other_part_no, partb, cells_info);
      create_partb_nodes_of_cell(noparts, max_gain, other_cell, other_part_no, partb, cells_info);
    } /* if */
...
/* update gain of a cell */
void update2(int flag, int noparts, int max_gain, int dest_part, int mov_cell_no, int cell_ptr, int net_no,
             int net_weight, nets_t nets[], corn_t ncells[], partb_t **partb, cells_info_t cells_info[],
             allele tchrom[]) {
  ...
  while ((i < nets[net_no].nno_cells) && (!found)) {
    ...
    if ((other_cell != mov_cell_no) && (cells_info[other_cell].locked == False) && (other_part_no != dest_part)) {
      found = True;
    } /* if */
  ...
  int mov_gain = calculate_gain(other_cell, other_part_no, dest_part, cells_info);
  int gain_inx = map_gain(mov_gain, max_gain);
  int mapped_part_no = map_part_no(dest_part, other_part_no);
  bnode_ptr_t tnode_ptr =
      delete_partb_node(False, mapped_part_no, &partb[other_part_no][mapped_part_no], &cells_info[other_cell]);
  insert_partb_node(tnode_ptr, mapped_part_no, gain_inx, &partb[other_part_no][mapped_part_no],
                    &cells_info[other_cell]);
} /* update2 */

void create_partb_nodes_of_cell(int noparts, int max_gain, int cell_no, int part_no, partb_t **partb,
                                cells_info_t cells_info[]) {
  if (cell_no == -1) {
    return;
  }
