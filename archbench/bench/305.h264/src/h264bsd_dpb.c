...
#define IS_SHORT_TERM(a) ((a).status == NON_EXISTING || (a).status == SHORT_TERM)
...
static u32 Mmcop3(dpbStorage_t *dpb, u32 currPicNum, u32 differenceOfPicNums, u32 longTermFrameIdx);

static u32 Mmcop4(dpbStorage_t *dpb, u32 maxLongTermFrameIdx);

static u32 Mmcop5(dpbStorage_t *dpb);

static u32 Mmcop6(dpbStorage_t *dpb, u32 frameNum, i32 picOrderCnt, u32 longTermFrameIdx);
...
u32 h264bsdReorderRefPicList(dpbStorage_t *dpb, refPicListReordering_t *order, u32 currFrameNum, u32 numRefIdxActive) {
  ...
  while (order->command[i].reorderingOfPicNumsIdc < 3) {
    ...
    if (order->command[i].reorderingOfPicNumsIdc < 2) {
      if (order->command[i].reorderingOfPicNumsIdc == 0) {
        picNumNoWrap = (i32)picNumPred - (i32)order->command[i].absDiffPicNum;
        if (picNumNoWrap < 0)
          picNumNoWrap += (i32)dpb->maxFrameNum;
      } else {
        picNumNoWrap = (i32)(picNumPred + order->command[i].absDiffPicNum);
...
static u32 Mmcop3(dpbStorage_t *dpb, u32 currPicNum, u32 differenceOfPicNums, u32 longTermFrameIdx) {
  ...
  if ((dpb->maxLongTermFrameIdx == NO_LONG_TERM_FRAME_INDICES) || (longTermFrameIdx > dpb->maxLongTermFrameIdx))
    return (HANTRO_NOK);
  ...
  for (i = 0; i < dpb->maxRefFrames; i++)
    if (IS_LONG_TERM(dpb->buffer[i]) && (u32)dpb->buffer[i].picNum == longTermFrameIdx) {
...
static u32 Mmcop4(dpbStorage_t *dpb, u32 maxLongTermFrameIdx) {
  ...
  for (i = 0; i < dpb->maxRefFrames; i++)
    if (IS_LONG_TERM(dpb->buffer[i]) && (((u32)dpb->buffer[i].picNum > maxLongTermFrameIdx) ||
...
u32 h264bsdMarkDecRefPic(dpbStorage_t *dpb, decRefPicMarking_t *mark, image_t *image, u32 frameNum, i32 picOrderCnt,
    ...
    if (mark->adaptiveRefPicMarkingModeFlag) {
      ...
      while (mark->operation[i].memoryManagementControlOperation) {
        switch (mark->operation[i].memoryManagementControlOperation) {
          case 1: status = Mmcop1(dpb, frameNum, mark->operation[i].differenceOfPicNums); break;

          case 2: status = Mmcop2(dpb, mark->operation[i].longTermPicNum); break;

          case 3:
            status = Mmcop3(dpb, frameNum, mark->operation[i].differenceOfPicNums, mark->operation[i].longTermFrameIdx);
            break;

          case 4: status = Mmcop4(dpb, mark->operation[i].maxLongTermFrameIdx); break;
          ...
          case 6:
            status = Mmcop6(dpb, frameNum, picOrderCnt, mark->operation[i].longTermFrameIdx);
            if (status == HANTRO_OK)
              markedAsLongTerm = HANTRO_TRUE;
            break;

          default: /* invalid memory management control operation */ status = HANTRO_NOK; break;
        }
        if (status != HANTRO_OK) {
          break;
        }
...
u8 *h264bsdAllocateDpbImage(dpbStorage_t *dpb) {
  ...
  ASSERT(!dpb->buffer[dpb->dpbSize].toBeDisplayed && !IS_REFERENCE(dpb->buffer[dpb->dpbSize]));
...
u32 h264bsdInitDpb(dpbStorage_t *dpb, u32 picSizeInMbs, u32 dpbSize, u32 maxRefFrames, u32 maxFrameNum,
                   u32 noReordering) {
  ...
  memset(dpb->buffer, 0, (MAX_NUM_REF_IDX_L0_ACTIVE + 1) * sizeof(dpbPicture_t));
  ...
  memset(dpb->list, 0, ((MAX_NUM_REF_IDX_L0_ACTIVE + 1) * sizeof(dpbPicture_t *)));
...
u32 h264bsdResetDpb(dpbStorage_t *dpb, u32 picSizeInMbs, u32 dpbSize, u32 maxRefFrames, u32 maxFrameNum,
                    u32 noReordering) {
  ...
  return h264bsdInitDpb(dpb, picSizeInMbs, dpbSize, maxRefFrames, maxFrameNum, noReordering);
}
...
static i32 FindDpbPic(dpbStorage_t *dpb, i32 picNum, u32 isShortTerm) {
  ...
  if (isShortTerm) {
    while (i < dpb->maxRefFrames && !found) {
      if (IS_SHORT_TERM(dpb->buffer[i]) && dpb->buffer[i].picNum == picNum)
        found = HANTRO_TRUE;
      else
        i++;
    }
    ...
    ASSERT(picNum >= 0);
    while (i < dpb->maxRefFrames && !found) {
      if (IS_LONG_TERM(dpb->buffer[i]) && dpb->buffer[i].picNum == picNum)
        found = HANTRO_TRUE;
      else
        i++;
    }
  ...
  for (i = 0; i < dpb->numRefFrames; i++)
    if (IS_SHORT_TERM(dpb->buffer[i])) {
      if (dpb->buffer[i].frameNum > currFrameNum)
        frameNumWrap = (i32)dpb->buffer[i].frameNum - (i32)dpb->maxFrameNum;
      else
...
u32 h264bsdCheckGapsInFrameNum(dpbStorage_t *dpb, u32 frameNum, u32 isRefPic, u32 gapsAllowed) {
  ...
  if ((frameNum != dpb->prevRefFrameNum) && (frameNum != ((dpb->prevRefFrameNum + 1) % dpb->maxFrameNum))) {
    ...
    do {
      ...
      unUsedShortTermFrameNum = (unUsedShortTermFrameNum + 1) % dpb->maxFrameNum;
        ...
        if (dpb->outBuf[i].data == dpb->buffer[dpb->dpbSize].data) {
          ...
          for (i = 0; i < dpb->dpbSize; i++) {
            if (dpb->buffer[i].data == tmp) {
              dpb->buffer[i].data = dpb->buffer[dpb->dpbSize].data;
...
dpbPicture_t *FindSmallestPicOrderCnt(dpbStorage_t *dpb) {
  ...
  for (i = 0; i <= dpb->dpbSize; i++) {
    if (dpb->buffer[i].toBeDisplayed && (dpb->buffer[i].picOrderCnt < picOrderCnt)) {
      tmp = dpb->buffer + i;
