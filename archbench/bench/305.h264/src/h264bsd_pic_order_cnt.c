...
i32 h264bsdDecodePicOrderCnt(pocStorage_t *poc, seqParamSet_t *sps, sliceHeader_t *pSliceHeader, nalUnit_t *pNalUnit) {
  ...
  switch (sps->picOrderCntType) {
    ...
    case 0:
      ...
      if ((pSliceHeader->picOrderCntLsb < poc->prevPicOrderCntLsb) &&
          ((poc->prevPicOrderCntLsb - pSliceHeader->picOrderCntLsb) >= sps->maxPicOrderCntLsb / 2)) {
        picOrderCnt = poc->prevPicOrderCntMsb + (i32)sps->maxPicOrderCntLsb;
      } else if ((pSliceHeader->picOrderCntLsb > poc->prevPicOrderCntLsb) &&
                 ((pSliceHeader->picOrderCntLsb - poc->prevPicOrderCntLsb) > sps->maxPicOrderCntLsb / 2)) {
        picOrderCnt = poc->prevPicOrderCntMsb - (i32)sps->maxPicOrderCntLsb;
      } else
      ...
      if (pNalUnit->nalRefIdc) {
        ...
        if (containsMmco5) {
          ...
          if (pSliceHeader->deltaPicOrderCntBottom < 0)
            poc->prevPicOrderCntLsb = (u32)(-pSliceHeader->deltaPicOrderCntBottom);
          else
            poc->prevPicOrderCntLsb = 0;
          picOrderCnt = 0;
        } else {
    ...
    case 1:
      ...
      if (absFrameNum > 0) {
        picOrderCntCycleCnt = (absFrameNum - 1) / sps->numRefFramesInPicOrderCntCycle;
        frameNumInPicOrderCntCycle = (absFrameNum - 1) % sps->numRefFramesInPicOrderCntCycle;
      }
      ...
      if (absFrameNum > 0) {
        picOrderCnt = (i32)picOrderCntCycleCnt * expectedDeltaPicOrderCntCycle;
    ...
    default: /* case 2 */
      ...
      if (IS_IDR_NAL_UNIT(pNalUnit))
        ...
        picOrderCnt = 2 * (i32)(frameNumOffset + pSliceHeader->frameNum) - 1;
      else
        picOrderCnt = 2 * (i32)(frameNumOffset + pSliceHeader->frameNum);
