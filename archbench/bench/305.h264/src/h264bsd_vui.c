...
static u32 DecodeHrdParameters(strmData_t *pStrmData, hrdParameters_t *pHrdParameters);
...
u32 h264bsdDecodeVuiParameters(strmData_t *pStrmData, vuiParameters_t *pVuiParameters) {
  ...
  if (pVuiParameters->videoSignalTypePresentFlag) {
    ...
    if (tmp == END_OF_STREAM)
      return (HANTRO_NOK);
    pVuiParameters->colourDescriptionPresentFlag = (tmp == 1) ? HANTRO_TRUE : HANTRO_FALSE;
  ...
  if (pVuiParameters->chromaLocInfoPresentFlag) {
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pVuiParameters->chromaSampleLocTypeTopField);
    if (tmp != HANTRO_OK)
      return (tmp);
    if (pVuiParameters->chromaSampleLocTypeTopField > 5)
      return (HANTRO_NOK);

    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pVuiParameters->chromaSampleLocTypeBottomField);
    if (tmp != HANTRO_OK)
      return (tmp);
  ...
  if (pVuiParameters->nalHrdParametersPresentFlag || pVuiParameters->vclHrdParametersPresentFlag) {
    tmp = h264bsdGetBits(pStrmData, 1);
    if (tmp == END_OF_STREAM)
      return (HANTRO_NOK);
...
static u32 DecodeHrdParameters(strmData_t *pStrmData, hrdParameters_t *pHrdParameters) {
  ...
  for (i = 0; i < pHrdParameters->cpbCnt; i++) {
    ...
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pHrdParameters->bitRateValue[i]);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    /* this may result in overflow, but this value is not used for
         * anything */
    pHrdParameters->bitRateValue[i] *= 1 << (6 + pHrdParameters->bitRateScale);

    /* cpb_size_value_minus1 in the range [0, 2^32 - 2] */
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pHrdParameters->cpbSizeValue[i]);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    /* this may result in overflow, but this value is not used for
         * anything */
    pHrdParameters->cpbSizeValue[i] *= 1 << (4 + pHrdParameters->cpbSizeScale);
