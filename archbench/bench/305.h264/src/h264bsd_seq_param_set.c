...
u32 h264bsdDecodeSeqParamSet(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet) {
  ...
  if (pSeqParamSet->picOrderCntType == 0) {
    ...
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSeqParamSet->numRefFramesInPicOrderCntCycle);
    if (tmp != HANTRO_OK)
      return (tmp);
  ...
  if (pSeqParamSet->frameCroppingFlag) {
    ...
    if (((i32)pSeqParamSet->frameCropLeftOffset >
         (8 * (i32)pSeqParamSet->picWidthInMbs - ((i32)pSeqParamSet->frameCropRightOffset + 1))) ||
        ((i32)pSeqParamSet->frameCropTopOffset >
         (8 * (i32)pSeqParamSet->picHeightInMbs - ((i32)pSeqParamSet->frameCropBottomOffset + 1)))) {
  ...
  if (pSeqParamSet->vuiParametersPresentFlag) {
    ...
    if (pSeqParamSet->vuiParameters->bitstreamRestrictionFlag) {
      if (pSeqParamSet->vuiParameters->numReorderFrames > pSeqParamSet->vuiParameters->maxDecFrameBuffering ||
...
u32 GetDpbSize(u32 picSizeInMbs, u32 levelIdc) {
  ...
  switch (levelIdc) {
    ...
    default: return (INVALID_DPB_SIZE);
  }
...
u32 h264bsdCompareSeqParamSets(seqParamSet_t *pSps1, seqParamSet_t *pSps2) {
  ...
  if (pSps1->profileIdc == pSps2->profileIdc && pSps1->levelIdc == pSps2->levelIdc &&
      pSps1->maxFrameNum == pSps2->maxFrameNum && pSps1->picOrderCntType == pSps2->picOrderCntType &&
      pSps1->numRefFrames == pSps2->numRefFrames &&
      pSps1->gapsInFrameNumValueAllowedFlag == pSps2->gapsInFrameNumValueAllowedFlag &&
      pSps1->picWidthInMbs == pSps2->picWidthInMbs && pSps1->picHeightInMbs == pSps2->picHeightInMbs &&
    ...
    if (pSps1->picOrderCntType == 0) {
      ...
      if (pSps1->deltaPicOrderAlwaysZeroFlag != pSps2->deltaPicOrderAlwaysZeroFlag ||
        ...
        for (i = 0; i < pSps1->numRefFramesInPicOrderCntCycle; i++)
          if (pSps1->offsetForRefFrame[i] != pSps2->offsetForRefFrame[i]) {
            return 1;
          }
      }
    }
