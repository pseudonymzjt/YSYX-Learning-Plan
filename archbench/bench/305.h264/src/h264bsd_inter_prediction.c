...
static u32 MvPrediction(mbStorage_t *pMb, subMbPred_t *subMbPred, u32 mbPartIdx, u32 subMbPartIdx);
static i32 MedianFilter(i32 a, i32 b, i32 c);

static void GetInterNeighbour(u32 sliceId, mbStorage_t *nMb, interNeighbour_t *n, u32 index);
static void GetPredictionMv(mv_t *mv, interNeighbour_t *a, u32 refIndex);

static const neighbour_t N_A_SUB_PART[4][4][4] = {{{{MB_A, 5}, {MB_NA, 0}, {MB_NA, 0}, {MB_NA, 0}},
...
static const neighbour_t N_B_SUB_PART[4][4][4] = {{{{MB_B, 10}, {MB_NA, 0}, {MB_NA, 0}, {MB_NA, 0}},
...
static const neighbour_t N_C_SUB_PART[4][4][4] = {{{{MB_B, 14}, {MB_NA, 0}, {MB_NA, 0}, {MB_NA, 0}},
...
static const neighbour_t N_D_SUB_PART[4][4][4] = {{{{MB_D, 15}, {MB_NA, 0}, {MB_NA, 0}, {MB_NA, 0}},
...
------------------------------------------------------------------------------*/
u32 h264bsdInterPrediction(mbStorage_t *pMb, macroblockLayer_t *pMbLayer, dpbStorage_t *dpb, u32 mbNum,
                           image_t *currImage, u8 *data) {
  ...
  switch (pMb->mbType) {
    ...
    case P_L0_16x16:
      ...
      h264bsdPredictSamples(data, pMb->mv, &refImage, colAndRow, tmp, pFill);
      break;
    ...
    case P_L0_L0_16x8:
      ...
      h264bsdPredictSamples(data, pMb->mv + 8, &refImage, colAndRow, tmp, pFill);
      break;
    ...
    case P_L0_L0_8x16:
      ...
      h264bsdPredictSamples(data, pMb->mv + 4, &refImage, colAndRow, tmp, pFill);
      break;
    ...
    default: /* P_8x8 and P_8x8ref0 */
      ...
      for (i = 0; i < 4; i++) {
        ...
        subPartMode = h264bsdSubMbPartMode(pMbLayer->subMbPred.subMbType[i]);
        x = i & 0x1 ? 8 : 0;
        y = i < 2 ? 0 : 8;
        switch (subPartMode) {
          case MB_SP_8x8:
            tmp = (x << 24) + (y << 16) + (8 << 8) + 8;
            h264bsdPredictSamples(data, pMb->mv + 4 * i, &refImage, colAndRow, tmp, pFill);
            break;
          ...
          case MB_SP_8x4:
            ...
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 2, &refImage, colAndRow, tmp, pFill);
            break;
          ...
          case MB_SP_4x8:
            ...
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 1, &refImage, colAndRow, tmp, pFill);
            break;
          ...
          default:
            ...
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 3, &refImage, colAndRow, tmp, pFill);
            break;
        }
      }
      break;
...
u32 h264bsdInterPrediction(mbStorage_t *pMb, macroblockLayer_t *pMbLayer, dpbStorage_t *dpb, u32 mbNum,
  ...
  switch (pMb->mbType) {
    ...
    case P_L0_16x16:
      if (MvPrediction16x16(pMb, &pMbLayer->mbPred, dpb) != HANTRO_OK)
        return (HANTRO_NOK);
      refImage.data = pMb->refAddr[0];
      h264bsdPredictSamples(data, pMb->mv, &refImage, col, row, 0, 0, 16, 16);
      break;
    ...
    case P_L0_L0_16x8:
      if (MvPrediction16x8(pMb, &pMbLayer->mbPred, dpb) != HANTRO_OK)
        return (HANTRO_NOK);
      refImage.data = pMb->refAddr[0];
      h264bsdPredictSamples(data, pMb->mv, &refImage, col, row, 0, 0, 16, 8);
      refImage.data = pMb->refAddr[2];
    ...
    case P_L0_L0_8x16:
      if (MvPrediction8x16(pMb, &pMbLayer->mbPred, dpb) != HANTRO_OK)
        return (HANTRO_NOK);
      refImage.data = pMb->refAddr[0];
      h264bsdPredictSamples(data, pMb->mv, &refImage, col, row, 0, 0, 8, 16);
      refImage.data = pMb->refAddr[1];
    ...
    default: /* P_8x8 and P_8x8ref0 */
      ...
      for (i = 0; i < 4; i++) {
        ...
        y = i < 2 ? 0 : 8;
        switch (subPartMode) {
          case MB_SP_8x8: h264bsdPredictSamples(data, pMb->mv + 4 * i, &refImage, col, row, x, y, 8, 8); break;

          case MB_SP_8x4:
            h264bsdPredictSamples(data, pMb->mv + 4 * i, &refImage, col, row, x, y, 8, 4);
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 2, &refImage, col, row, x, y + 4, 8, 4);
            break;

          case MB_SP_4x8:
            h264bsdPredictSamples(data, pMb->mv + 4 * i, &refImage, col, row, x, y, 4, 8);
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 1, &refImage, col, row, x + 4, y, 4, 8);
            break;

          default:
            h264bsdPredictSamples(data, pMb->mv + 4 * i, &refImage, col, row, x, y, 4, 4);
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 1, &refImage, col, row, x + 4, y, 4, 4);
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 2, &refImage, col, row, x, y + 4, 4, 4);
            h264bsdPredictSamples(data, pMb->mv + 4 * i + 3, &refImage, col, row, x + 4, y + 4, 4, 4);
            break;
        }
      }
      break;
  ...
  if (pMb->mbType != P_Skip) {
    h264bsdWriteOutputBlocks(currImage, mbNum, data, pMbLayer->residual.level);
  } else {
...
u32 MvPrediction16x16(mbStorage_t *pMb, mbPred_t *mbPred, dpbStorage_t *dpb) {
  ...
  if (pMb->mbType == P_Skip && (!a[0].available || !a[1].available || (a[0].refIndex == 0 && ((u32)(*tmpMv1) == 0)) ||
  ...
  pMb->mv[0] = pMb->mv[1] = pMb->mv[2] = pMb->mv[3] = pMb->mv[4] = pMb->mv[5] = pMb->mv[6] = pMb->mv[7] = pMb->mv[8] =
      pMb->mv[9] = pMb->mv[10] = pMb->mv[11] = pMb->mv[12] = pMb->mv[13] = pMb->mv[14] = pMb->mv[15] = mv;
...
u32 MvPrediction16x8(mbStorage_t *pMb, mbPred_t *mbPred, dpbStorage_t *dpb) {
  ...
  pMb->mv[0] = pMb->mv[1] = pMb->mv[2] = pMb->mv[3] = pMb->mv[4] = pMb->mv[5] = pMb->mv[6] = pMb->mv[7] = mv;
  pMb->refPic[0] = refIndex;
  pMb->refPic[1] = refIndex;
  pMb->refAddr[0] = tmp;
  pMb->refAddr[1] = tmp;
  ...
  pMb->mv[8] = pMb->mv[9] = pMb->mv[10] = pMb->mv[11] = pMb->mv[12] = pMb->mv[13] = pMb->mv[14] = pMb->mv[15] = mv;
  pMb->refPic[2] = refIndex;
  pMb->refPic[3] = refIndex;
  pMb->refAddr[2] = tmp;
  pMb->refAddr[3] = tmp;
...
u32 MvPrediction8x16(mbStorage_t *pMb, mbPred_t *mbPred, dpbStorage_t *dpb) {
  ...
  pMb->mv[0] = pMb->mv[1] = pMb->mv[2] = pMb->mv[3] = pMb->mv[8] = pMb->mv[9] = pMb->mv[10] = pMb->mv[11] = mv;
  pMb->refPic[0] = refIndex;
  pMb->refPic[2] = refIndex;
  pMb->refAddr[0] = tmp;
  pMb->refAddr[2] = tmp;
  ...
  pMb->mv[4] = pMb->mv[5] = pMb->mv[6] = pMb->mv[7] = pMb->mv[12] = pMb->mv[13] = pMb->mv[14] = pMb->mv[15] = mv;
  pMb->refPic[1] = refIndex;
  pMb->refPic[3] = refIndex;
  pMb->refAddr[1] = tmp;
  pMb->refAddr[3] = tmp;
...
u32 MvPrediction(mbStorage_t *pMb, subMbPred_t *subMbPred, u32 mbPartIdx, u32 subMbPartIdx) {
  ...
  switch (subMbPartMode) {
    ...
    case MB_SP_4x4: pMb->mv[4 * mbPartIdx + subMbPartIdx] = mv; break;
  }
...
void GetInterNeighbour(u32 sliceId, mbStorage_t *nMb, interNeighbour_t *n, u32 index) {
