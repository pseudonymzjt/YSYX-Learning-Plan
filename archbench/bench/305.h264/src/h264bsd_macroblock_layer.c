...
static const u32 lumaIndex[16] = {0, 4, 64, 68, 8, 12, 72, 76, 128, 132, 192, 196, 136, 140, 200, 204};
#endif
/* mapping of dc coefficients array to luma blocks */
static const u32 dcCoeffIndex[16] = {0, 1, 4, 5, 2, 3, 6, 7, 8, 9, 12, 13, 10, 11, 14, 15};

/*------------------------------------------------------------------------------
    4. Local function prototypes
------------------------------------------------------------------------------*/

static u32 DecodeMbPred(strmData_t *pStrmData, mbPred_t *pMbPred, mbType_e mbType, u32 numRefIdxActive);
static u32 DecodeSubMbPred(strmData_t *pStrmData, subMbPred_t *pSubMbPred, mbType_e mbType, u32 numRefIdxActive);
static u32 DecodeResidual(strmData_t *pStrmData, residual_t *pResidual, mbStorage_t *pMb, mbType_e mbType,
...
#ifdef H264DEC_OMXDL
static u32 ProcessIntra4x4Residual(mbStorage_t *pMb, u8 *data, u32 constrainedIntraPred, macroblockLayer_t *mbLayer,
                                   const u8 **pSrc, image_t *image);
static u32 ProcessChromaResidual(mbStorage_t *pMb, u8 *data, const u8 **pSrc);
static u32 ProcessIntra16x16Residual(mbStorage_t *pMb, u8 *data, u32 constrainedIntraPred, u32 intraChromaPredMode,
                                     const u8 **pSrc, image_t *image);
...
u32 h264bsdDecodeMacroblockLayer(strmData_t *pStrmData, macroblockLayer_t *pMbLayer, mbStorage_t *pMb, u32 sliceType,
                                 u32 numRefIdxActive) {
  ...
  if (pMbLayer->mbType == I_PCM) {
    ...
    if ((partMode == PRED_MODE_INTER) && (h264bsdNumMbPart(pMbLayer->mbType) == 4)) {
      tmp = DecodeSubMbPred(pStrmData, &pMbLayer->subMbPred, pMbLayer->mbType, numRefIdxActive);
    } else {
    ...
    if (partMode != PRED_MODE_INTRA16x16) {
      tmp = h264bsdDecodeExpGolombMapped(pStrmData, &value, (u32)(partMode == PRED_MODE_INTRA4x4));
      if (tmp != HANTRO_OK)
        return (tmp);
    ...
    if (pMbLayer->codedBlockPattern || (partMode == PRED_MODE_INTRA16x16)) {
      ...
      tmp = DecodeResidual(pStrmData, &pMbLayer->residual, pMb, pMbLayer->mbType, pMbLayer->codedBlockPattern);

      pStrmData->strmBuffReadBits =
          (u32)(pStrmData->pStrmCurrPos - pStrmData->pStrmBuffStart) * 8 + pStrmData->bitPosInWord;
...
u32 DecodeMbPred(strmData_t *pStrmData, mbPred_t *pMbPred, mbType_e mbType, u32 numRefIdxActive) {
  ...
  switch (h264bsdMbPartPredMode(mbType)) {
    case PRED_MODE_INTER: /* PRED_MODE_INTER */
      if (numRefIdxActive > 1) {
        for (i = h264bsdNumMbPart(mbType), j = 0; i--; j++) {
          tmp = h264bsdDecodeExpGolombTruncated(pStrmData, &value, (u32)(numRefIdxActive > 2));
    ...
    case PRED_MODE_INTRA4x4:
      for (itmp = 0, i = 0; itmp < 2; itmp++) {
        ...
        tmp = 0;
        for (j = 8; j--; i++) {
          pMbPred->prevIntra4x4PredModeFlag[i] = value & 0x80000000 ? HANTRO_TRUE : HANTRO_FALSE;
          value <<= 1;
...
u32 DecodeSubMbPred(strmData_t *pStrmData, subMbPred_t *pSubMbPred, mbType_e mbType, u32 numRefIdxActive) {
  ...
  for (i = 0; i < 4; i++) {
    j = 0;
    for (value = h264bsdNumSubMbPart(pSubMbPred->subMbType[i]); value--; j++) {
      tmp = h264bsdDecodeExpGolombSigned(pStrmData, &itmp);
      if (tmp != HANTRO_OK)
        return (tmp);
...
u32 DecodeResidual(strmData_t *pStrmData, residual_t *pResidual, mbStorage_t *pMb, mbType_e mbType,
                   u32 codedBlockPattern) {
...
#ifndef H264DEC_NEON
    omxRes = omxVCM4P10_DecodeCoeffsToPairCAVLC((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                (OMX_S32 *)(&pStrmData->bitPosInWord), &pResidual->totalCoeff[24],
                                                &pPosCoefBuf, nc, 16);
#else
    omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                           (OMX_S32 *)(&pStrmData->bitPosInWord), &pResidual->totalCoeff[24],
                                           &pPosCoefBuf, nc, 16);
#endif
      ...
      for (j = 4; j--; blockIndex++) {
        ...
        if (is16x16) {
          ...
          omxRes = omxVCM4P10_DecodeCoeffsToPairCAVLC((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                      ...
                                                      &pResidual->totalCoeff[blockIndex], &pPosCoefBuf, nc, 15);
#else
          omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                 (OMX_S32 *)(&pStrmData->bitPosInWord),
                                                 &pResidual->totalCoeff[blockIndex], &pPosCoefBuf, nc, 15);
          ...
          omxRes = omxVCM4P10_DecodeCoeffsToPairCAVLC((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                      (OMX_S32 *)(&pStrmData->bitPosInWord),
                                                      &pResidual->totalCoeff[blockIndex], &pPosCoefBuf, nc, 16);
#else
          omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                 (OMX_S32 *)(&pStrmData->bitPosInWord),
                                                 &pResidual->totalCoeff[blockIndex], &pPosCoefBuf, nc, 16);
  ...
  if (blockCoded) {
#ifndef H264DEC_NEON
    omxRes = omxVCM4P10_DecodeChromaDcCoeffsToPairCAVLC((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                        (OMX_S32 *)(&pStrmData->bitPosInWord),
                                                        &pResidual->totalCoeff[25], &pPosCoefBuf);
#else
    omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                           (OMX_S32 *)(&pStrmData->bitPosInWord), &pResidual->totalCoeff[25],
                                           &pPosCoefBuf, 17, 4);
#endif
    if (omxRes != OMX_Sts_NoErr)
      return (HANTRO_NOK);
    ...
    omxRes = omxVCM4P10_DecodeChromaDcCoeffsToPairCAVLC((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                                        (OMX_S32 *)(&pStrmData->bitPosInWord),
                                                        &pResidual->totalCoeff[26], &pPosCoefBuf);
#else
    omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                           (OMX_S32 *)(&pStrmData->bitPosInWord), &pResidual->totalCoeff[26],
                                           &pPosCoefBuf, 17, 4);
#endif
    if (omxRes != OMX_Sts_NoErr)
      return (HANTRO_NOK);
...
#else
      omxRes = armVCM4P10_DecodeCoeffsToPair((const OMX_U8 **)(&pStrmData->pStrmCurrPos),
                                             (OMX_S32 *)(&pStrmData->bitPosInWord), &pResidual->totalCoeff[blockIndex],
                                             &pPosCoefBuf, nc, 15);
#endif
      if (omxRes != OMX_Sts_NoErr)
        return (HANTRO_NOK);
  ...
  for (i = 4, blockIndex = 0; i--;) {
    ...
    if (blockCoded) {
      for (j = 4; j--; blockIndex++) {
        ...
        if (is16x16) {
          tmp = h264bsdDecodeResidualBlockCavlc(pStrmData, level[blockIndex] + 1, nc, 15);
          pResidual->coeffMap[blockIndex] = tmp >> 15;
        } else {
          tmp = h264bsdDecodeResidualBlockCavlc(pStrmData, level[blockIndex], nc, 16);
...
u32 h264bsdDecodeMacroblock(mbStorage_t *pMb, macroblockLayer_t *pMbLayer, image_t *currImage, dpbStorage_t *dpb,
                            i32 *qpY, u32 mbNum, u32 constrainedIntraPredFlag, u8 *data) {
  ...
  if (mbType == I_PCM) {
    ...
    if (h264bsdMbPartPredMode(mbType) == PRED_MODE_INTER) {
      tmp = h264bsdInterPrediction(pMb, pMbLayer, dpb, mbNum, currImage, (u8 *)data);
      if (tmp != HANTRO_OK)
        return (tmp);
    }
#endif
    if (mbType != P_Skip) {
      memcpy(pMb->totalCoeff, pMbLayer->residual.totalCoeff, 27 * sizeof(*pMb->totalCoeff));
      ...
      if (h264bsdMbPartPredMode(mbType) == PRED_MODE_INTER) {
        ...
        for (i = 0; i < 16; i++, totalCoeff++) {
          p = data + lumaIndex[i];
          if (*totalCoeff) {
            res = omxVCM4P10_DequantTransformResidualFromPairAndAdd(&pSrc, p, 0, p, 16, 16, *qpY, *totalCoeff);
        ...
        tmp = ProcessIntra4x4Residual(pMb, data, constrainedIntraPredFlag, pMbLayer, &pSrc, currImage);
        if (tmp != HANTRO_OK)
          return (tmp);
      } else if (h264bsdMbPartPredMode(mbType) == PRED_MODE_INTRA16x16) {
        tmp = ProcessIntra16x16Residual(pMb, data, constrainedIntraPredFlag, pMbLayer->mbPred.intraChromaPredMode,
                                        &pSrc, currImage);
        if (tmp != HANTRO_OK)
          return (tmp);
      }
...
#else
    if (h264bsdMbPartPredMode(mbType) != PRED_MODE_INTER) {
      tmp = h264bsdIntraPrediction(pMb, pMbLayer, currImage, mbNum, constrainedIntraPredFlag, (u8 *)data);
      if (tmp != HANTRO_OK)
        return (tmp);
    } else {
      tmp = h264bsdInterPrediction(pMb, pMbLayer, dpb, mbNum, currImage, (u8 *)data);
...
u32 ProcessChromaResidual(mbStorage_t *pMb, u8 *data, const u8 **pSrc) {
  ...
  if (pMb->totalCoeff[25]) {
    pDc = dc;
    result = omxVCM4P10_TransformDequantChromaDCFromPair(pSrc, pDc, (i32)chromaQp);
  ...
  for (i = 0; i < 8; i++, pDc++, totalCoeff++) {
    ...
    if (*totalCoeff || *pDc) {
      ...
      result = omxVCM4P10_DequantTransformResidualFromPairAndAdd(pSrc, p, pDc, p, 8, 8, (i32)chromaQp, *totalCoeff);
      if (result != OMX_Sts_NoErr)
        return (HANTRO_NOK);
    }
  }
...
------------------------------------------------------------------------------*/
u32 ProcessIntra16x16Residual(mbStorage_t *pMb, u8 *data, u32 constrainedIntraPred, u32 intraChromaPredMode,
                              const u8 **pSrc, image_t *image) {
  u32 i;
  i16 *pDc;
  ...
  if (totalCoeff[24]) {
    pDc = dc;
    result = omxVCM4P10_TransformDequantLumaDCFromPair(pSrc, pDc, (i32)pMb->qpY);
    if (result != OMX_Sts_NoErr)
      return (HANTRO_NOK);
  }
  /* Intra 16x16 pred */
  if (h264bsdIntra16x16Prediction(pMb, data, image->luma, image->width * 16, constrainedIntraPred) != HANTRO_OK)
    return (HANTRO_NOK);
  for (i = 0; i < 16; i++, totalCoeff++) {
    p = data + lumaIndex[i];
    pDc = &dc[dcCoeffIndex[i]];
    if (*totalCoeff || *pDc) {
      result = omxVCM4P10_DequantTransformResidualFromPairAndAdd(pSrc, p, pDc, p, 16, 16, (i32)pMb->qpY, *totalCoeff);
      if (result != OMX_Sts_NoErr)
        return (HANTRO_NOK);
    }
  }

  if (h264bsdIntraChromaPrediction(pMb, data + 256, image, intraChromaPredMode, constrainedIntraPred) != HANTRO_OK)
    return (HANTRO_NOK);
...
------------------------------------------------------------------------------*/
u32 ProcessIntra4x4Residual(mbStorage_t *pMb, u8 *data, u32 constrainedIntraPred, macroblockLayer_t *mbLayer,
                            const u8 **pSrc, image_t *image) {
  u32 i;
  u8 *totalCoeff;
  OMXResult result;
  ...
  for (i = 0; i < 16; i++, totalCoeff++) {
    p = data + lumaIndex[i];
    if (h264bsdIntra4x4Prediction(pMb, p, mbLayer, image->luma, image->width * 16, constrainedIntraPred, i) !=
        HANTRO_OK)
      return (HANTRO_NOK);

    if (*totalCoeff) {
      result = omxVCM4P10_DequantTransformResidualFromPairAndAdd(pSrc, p, NULL, p, 16, 16, (i32)pMb->qpY, *totalCoeff);
      if (result != OMX_Sts_NoErr)
        return (HANTRO_NOK);
    }
  }

  if (h264bsdIntraChromaPrediction(pMb, data + 256, image, mbLayer->mbPred.intraChromaPredMode, constrainedIntraPred) !=
      HANTRO_OK)
    return (HANTRO_NOK);
...
u32 ProcessResidual(mbStorage_t *pMb, i32 residualLevel[][16], u32 *coeffMap) {
  ...
  if (h264bsdMbPartPredMode(pMb->mbType) == PRED_MODE_INTRA16x16) {
    ...
    for (i = 16; i--; blockData++, totalCoeff++, coeffMap++) {
      ...
      if ((*blockData)[0] || *totalCoeff) {
        if (h264bsdProcessBlock(*blockData, pMb->qpY, 1, *coeffMap) != HANTRO_OK)
          return (HANTRO_NOK);
      } else
  ...
  for (i = 8; i--; blockData++, totalCoeff++, coeffMap++) {
    ...
    if ((*blockData)[0] || *totalCoeff) {
      if (h264bsdProcessBlock(*blockData, chromaQp, 1, *coeffMap) != HANTRO_OK)
        return (HANTRO_NOK);
    } else
      MARK_RESIDUAL_EMPTY(*blockData);
  }
