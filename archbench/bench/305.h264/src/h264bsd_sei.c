...
static u32 DecodeBufferingPeriod(strmData_t *pStrmData, seiBufferingPeriod_t *pBufferingPeriod, u32 cpbCnt,
                                 u32 initialCpbRemovalDelayLength, u32 nalHrdBpPresentFlag, u32 vclHrdBpPresentFlag);

static u32 DecodePictureTiming(strmData_t *pStrmData, seiPicTiming_t *pPicTiming, u32 cpbRemovalDelayLength,
                               u32 dpbOutputDelayLength, u32 timeOffsetLength, u32 cpbDpbDelaysPresentFlag,
                               u32 picStructPresentFlag);

static u32 DecodePanScanRectangle(strmData_t *pStrmData, seiPanScanRect_t *pPanScanRectangle);

static u32 DecodeFillerPayload(strmData_t *pStrmData, u32 payloadSize);

static u32 DecodeUserDataRegisteredITuTT35(strmData_t *pStrmData,
                                           seiUserDataRegisteredItuTT35_t *pUserDataRegisteredItuTT35, u32 payloadSize);

static u32 DecodeUserDataUnregistered(strmData_t *pStrmData, seiUserDataUnregistered_t *pUserDataUnregistered,
                                      u32 payloadSize);

static u32 DecodeRecoveryPoint(strmData_t *pStrmData, seiRecoveryPoint_t *pRecoveryPoint);

static u32 DecodeDecRefPicMarkingRepetition(strmData_t *pStrmData,
                                            seiDecRefPicMarkingRepetition_t *pDecRefPicMarkingRepetition,
                                            u32 numRefFrames);

static u32 DecodeSparePic(strmData_t *pStrmData, seiSparePic_t *pSparePic, u32 picSizeInMapUnits);

static u32 DecodeSceneInfo(strmData_t *pStrmData, seiSceneInfo_t *pSceneInfo);

static u32 DecodeSubSeqInfo(strmData_t *pStrmData, seiSubSeqInfo_t *pSubSeqInfo);

static u32 DecodeSubSeqLayerCharacteristics(strmData_t *pStrmData,
                                            seiSubSeqLayerCharacteristics_t *pSubSeqLayerCharacteristics);

static u32 DecodeSubSeqCharacteristics(strmData_t *pStrmData, seiSubSeqCharacteristics_t *pSubSeqCharacteristics);

static u32 DecodeFullFrameFreeze(strmData_t *pStrmData, seiFullFrameFreeze_t *pFullFrameFreeze);

static u32 DecodeFullFrameSnapshot(strmData_t *pStrmData, seiFullFrameSnapshot_t *pFullFrameSnapshot);

static u32
DecodeProgressiveRefinementSegmentStart(strmData_t *pStrmData,
                                        seiProgressiveRefinementSegmentStart_t *pProgressiveRefinementSegmentStart);

static u32
DecodeProgressiveRefinementSegmentEnd(strmData_t *pStrmData,
                                      seiProgressiveRefinementSegmentEnd_t *pProgressiveRefinementSegmentEnd);

static u32 DecodeMotionConstrainedSliceGroupSet(strmData_t *pStrmData,
                                                seiMotionConstrainedSliceGroupSet_t *pMotionConstrainedSliceGroupSet,
                                                u32 numSliceGroups);

static u32 DecodeReservedSeiMessage(strmData_t *pStrmData, seiReservedSeiMessage_t *pReservedSeiMessage,
                                    u32 payloadSize);
...
u32 h264bsdDecodeSeiMessage(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet, seiMessage_t *pSeiMessage,
                            u32 numSliceGroups) {
  ...
  do {
    ...
    switch (payloadType) {
      case 0:
        ASSERT(pSeqParamSet);
        status = DecodeBufferingPeriod(pStrmData, &pSeiMessage->bufferingPeriod,
      ...
      case 1:
        ...
        status = DecodePictureTiming(pStrmData, &pSeiMessage->picTiming,
      ...
      case 2: status = DecodePanScanRectangle(pStrmData, &pSeiMessage->panScanRect); break;

      case 3: status = DecodeFillerPayload(pStrmData, payloadSize); break;

      case 4:
        status = DecodeUserDataRegisteredITuTT35(pStrmData, &pSeiMessage->userDataRegisteredItuTT35, payloadSize);
        break;

      case 5: status = DecodeUserDataUnregistered(pStrmData, &pSeiMessage->userDataUnregistered, payloadSize); break;

      case 6: status = DecodeRecoveryPoint(pStrmData, &pSeiMessage->recoveryPoint); break;

      case 7:
        status = DecodeDecRefPicMarkingRepetition(pStrmData, &pSeiMessage->decRefPicMarkingRepetition,
      ...
      case 9: status = DecodeSceneInfo(pStrmData, &pSeiMessage->sceneInfo); break;

      case 10: status = DecodeSubSeqInfo(pStrmData, &pSeiMessage->subSeqInfo); break;

      case 11: status = DecodeSubSeqLayerCharacteristics(pStrmData, &pSeiMessage->subSeqLayerCharacteristics); break;

      case 12: status = DecodeSubSeqCharacteristics(pStrmData, &pSeiMessage->subSeqCharacteristics); break;

      case 13: status = DecodeFullFrameFreeze(pStrmData, &pSeiMessage->fullFrameFreeze); break;

      case 14: /* This SEI does not contain data, what to do ??? */ status = HANTRO_OK; break;

      case 15: status = DecodeFullFrameSnapshot(pStrmData, &pSeiMessage->fullFrameSnapshot); break;

      case 16:
        status = DecodeProgressiveRefinementSegmentStart(pStrmData, &pSeiMessage->progressiveRefinementSegmentStart);
        break;

      case 17:
        status = DecodeProgressiveRefinementSegmentEnd(pStrmData, &pSeiMessage->progressiveRefinementSegmentEnd);
        break;

      case 18:
        ASSERT(numSliceGroups);
        status = DecodeMotionConstrainedSliceGroupSet(pStrmData, &pSeiMessage->motionConstrainedSliceGroupSet,
                                                      numSliceGroups);
        break;

      default: status = DecodeReservedSeiMessage(pStrmData, &pSeiMessage->reservedSeiMessage, payloadSize); break;
    }
...
static u32 DecodeBufferingPeriod(strmData_t *pStrmData, seiBufferingPeriod_t *pBufferingPeriod, u32 cpbCnt,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pBufferingPeriod->seqParameterSetId);
  if (tmp != HANTRO_OK)
    return (tmp);
...
static u32 DecodePictureTiming(strmData_t *pStrmData, seiPicTiming_t *pPicTiming, u32 cpbRemovalDelayLength,
  ...
  if (picStructPresentFlag) {
    ...
    for (i = 0; i < numClockTS[pPicTiming->picStruct]; i++) {
      ...
      if (tmp == END_OF_STREAM)
        return (HANTRO_NOK);
      pPicTiming->clockTimeStampFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
      ...
      if (pPicTiming->clockTimeStampFlag[i]) {
        ...
        if (tmp == END_OF_STREAM)
          return (HANTRO_NOK);
        pPicTiming->nuitFieldBasedFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
        ...
        if (tmp == END_OF_STREAM)
          return (HANTRO_NOK);
        pPicTiming->fullTimeStampFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;

        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM)
          return (HANTRO_NOK);
        pPicTiming->discontinuityFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;

        tmp = h264bsdGetBits(pStrmData, 1);
        if (tmp == END_OF_STREAM)
          return (HANTRO_NOK);
        pPicTiming->cntDroppedFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
        ...
        if (pPicTiming->fullTimeStampFlag[i]) {
          ...
          if (tmp == END_OF_STREAM)
            return (HANTRO_NOK);
          pPicTiming->secondsFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
          ...
          if (pPicTiming->secondsFlag[i]) {
            ...
            if (tmp == END_OF_STREAM)
              return (HANTRO_NOK);
            pPicTiming->minutesFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
            ...
            if (pPicTiming->minutesFlag[i]) {
              ...
              if (tmp == END_OF_STREAM)
                return (HANTRO_NOK);
              pPicTiming->hoursFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
...
static u32 DecodePanScanRectangle(strmData_t *pStrmData, seiPanScanRect_t *pPanScanRectangle) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pPanScanRectangle->panScanRectId);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  if (tmp == END_OF_STREAM)
    return (HANTRO_NOK);
  pPanScanRectangle->panScanRectCancelFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;

  if (!pPanScanRectangle->panScanRectCancelFlag) {
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pPanScanRectangle->panScanCnt);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    for (i = 0; i < pPanScanRectangle->panScanCnt; i++) {
      tmp = h264bsdDecodeExpGolombSigned(pStrmData, &pPanScanRectangle->panScanRectLeftOffset[i]);
      if (tmp != HANTRO_OK)
        return (tmp);

      tmp = h264bsdDecodeExpGolombSigned(pStrmData, &pPanScanRectangle->panScanRectRightOffset[i]);
      if (tmp != HANTRO_OK)
        return (tmp);

      tmp = h264bsdDecodeExpGolombSigned(pStrmData, &pPanScanRectangle->panScanRectTopOffset[i]);
      if (tmp != HANTRO_OK)
        return (tmp);

      tmp = h264bsdDecodeExpGolombSigned(pStrmData, &pPanScanRectangle->panScanRectBottomOffset[i]);
      if (tmp != HANTRO_OK)
        return (tmp);
    }
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pPanScanRectangle->panScanRectRepetitionPeriod);
    if (tmp != HANTRO_OK)
      return (tmp);
    if (pPanScanRectangle->panScanRectRepetitionPeriod > 16384)
      return (HANTRO_NOK);
    if (pPanScanRectangle->panScanCnt > 1 && pPanScanRectangle->panScanRectRepetitionPeriod > 1)
      return (HANTRO_NOK);
  }
...
static u32 DecodeRecoveryPoint(strmData_t *pStrmData, seiRecoveryPoint_t *pRecoveryPoint) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pRecoveryPoint->recoveryFrameCnt);
  if (tmp != HANTRO_OK)
    return (tmp);
...
static u32 DecodeDecRefPicMarkingRepetition(strmData_t *pStrmData,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pDecRefPicMarkingRepetition->originalFrameNum);
  if (tmp != HANTRO_OK)
    return (tmp);
...
static u32 DecodeSparePic(strmData_t *pStrmData, seiSparePic_t *pSparePic, u32 picSizeInMapUnits) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSparePic->targetFrameNum);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  for (i = 0; i < pSparePic->numSparePics; i++) {
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSparePic->deltaSpareFrameNum[i]);
    if (tmp != HANTRO_OK)
      return (tmp);

    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSparePic->spareAreaIdc[i]);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    if (pSparePic->spareAreaIdc[i] == 1) {
      ...
      for (j = 0; j < picSizeInMapUnits; j++) {
        ...
        if (tmp == END_OF_STREAM)
          return (HANTRO_NOK);
        pSparePic->spareUnitFlag[i][j] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
      }
      ...
      for (j = 0, mapUnitCnt = 0; mapUnitCnt < picSizeInMapUnits; j++) {
        tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSparePic->zeroRunLength[i][j]);
        if (tmp != HANTRO_OK)
          return (tmp);
...
static u32 DecodeSceneInfo(strmData_t *pStrmData, seiSceneInfo_t *pSceneInfo) {
  ...
  if (pSceneInfo->sceneInfoPresentFlag) {
    ...
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSceneInfo->sceneTransitionType);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    if (pSceneInfo->sceneTransitionType) {
      tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSceneInfo->secondSceneId);
      if (tmp != HANTRO_OK)
        return (tmp);
    }
  }
...
static u32 DecodeSubSeqInfo(strmData_t *pStrmData, seiSubSeqInfo_t *pSubSeqInfo) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqInfo->subSeqLayerNum);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  if (pSubSeqInfo->subSeqFrameNumFlag) {
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqInfo->subSeqFrameNum);
    if (tmp != HANTRO_OK)
      return (tmp);
  }
...
static u32 DecodeSubSeqLayerCharacteristics(strmData_t *pStrmData,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqLayerCharacteristics->numSubSeqLayers);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  for (i = 0; i < pSubSeqLayerCharacteristics->numSubSeqLayers; i++) {
    ...
    if (tmp == END_OF_STREAM)
      return (HANTRO_NOK);
    pSubSeqLayerCharacteristics->accurateStatisticsFlag[i] = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
...
static u32 DecodeSubSeqCharacteristics(strmData_t *pStrmData, seiSubSeqCharacteristics_t *pSubSeqCharacteristics) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqCharacteristics->subSeqLayerNum);
  if (tmp != HANTRO_OK)
    return (tmp);
  if (pSubSeqCharacteristics->subSeqLayerNum > MAX_NUM_SUB_SEQ_LAYERS - 1)
    return (HANTRO_NOK);

  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqCharacteristics->subSeqId);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  if (tmp == END_OF_STREAM)
    return (HANTRO_NOK);
  pSubSeqCharacteristics->durationFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
  ...
  if (tmp == END_OF_STREAM)
    return (HANTRO_NOK);
  pSubSeqCharacteristics->averageRateFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;

  if (pSubSeqCharacteristics->averageRateFlag) {
    tmp = h264bsdGetBits(pStrmData, 1);
    if (tmp == END_OF_STREAM)
      return (HANTRO_NOK);
    pSubSeqCharacteristics->accurateStatisticsFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqCharacteristics->numReferencedSubseqs);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  for (i = 0; i < pSubSeqCharacteristics->numReferencedSubseqs; i++) {
    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqCharacteristics->refSubSeqLayerNum[i]);
    if (tmp != HANTRO_OK)
      return (tmp);

    tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pSubSeqCharacteristics->refSubSeqId[i]);
    if (tmp != HANTRO_OK)
      return (tmp);
...
static u32 DecodeFullFrameFreeze(strmData_t *pStrmData, seiFullFrameFreeze_t *pFullFrameFreeze) {
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pFullFrameFreeze->fullFrameFreezeRepetitionPeriod);
  if (tmp != HANTRO_OK)
    return (tmp);
...
DecodeProgressiveRefinementSegmentStart(strmData_t *pStrmData,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pProgressiveRefinementSegmentStart->progressiveRefinementId);
  if (tmp != HANTRO_OK)
    return (tmp);

  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pProgressiveRefinementSegmentStart->numRefinementSteps);
  if (tmp != HANTRO_OK)
    return (tmp);
...
DecodeProgressiveRefinementSegmentEnd(strmData_t *pStrmData,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pProgressiveRefinementSegmentEnd->progressiveRefinementId);
  if (tmp != HANTRO_OK)
    return (tmp);
...
static u32 DecodeMotionConstrainedSliceGroupSet(strmData_t *pStrmData,
  ...
  tmp = h264bsdDecodeExpGolombUnsigned(pStrmData, &pMotionConstrainedSliceGroupSet->numSliceGroupsInSet);
  if (tmp != HANTRO_OK)
    return (tmp);
  ...
  for (i = 0; i < pMotionConstrainedSliceGroupSet->numSliceGroupsInSet; i++) {
    tmp = h264bsdGetBits(pStrmData, ceilLog2NumSliceGroups[numSliceGroups]);
    if (tmp == END_OF_STREAM)
      return (HANTRO_NOK);
    pMotionConstrainedSliceGroupSet->sliceGroupId[i] = tmp;
    if (pMotionConstrainedSliceGroupSet->sliceGroupId[i] > pMotionConstrainedSliceGroupSet->numSliceGroupsInSet - 1)
      return (HANTRO_NOK);
  }
  ...
  if (tmp == END_OF_STREAM)
    return (HANTRO_NOK);
  pMotionConstrainedSliceGroupSet->exactSampleValueMatchFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;

  tmp = h264bsdGetBits(pStrmData, 1);
  if (tmp == END_OF_STREAM)
    return (HANTRO_NOK);
  pMotionConstrainedSliceGroupSet->panScanRectFlag = tmp == 1 ? HANTRO_TRUE : HANTRO_FALSE;
