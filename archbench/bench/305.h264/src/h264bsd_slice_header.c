...
static u32 RefPicListReordering(strmData_t *, refPicListReordering_t *, u32, u32);

static u32 NumSliceGroupChangeCycleBits(u32 picSizeInMbs, u32 sliceGroupChangeRate);

static u32 DecRefPicMarking(strmData_t *pStrmData, decRefPicMarking_t *pDecRefPicMarking, nalUnitType_e nalUnitType,
                            u32 numRefFrames);
...
u32 h264bsdDecodeSliceHeader(strmData_t *pStrmData, sliceHeader_t *pSliceHeader, seqParamSet_t *pSeqParamSet,
  ...
  ASSERT(pSeqParamSet);
  ASSERT(pPicParamSet);
  ASSERT(pNalUnit->nalUnitType == NAL_CODED_SLICE || pNalUnit->nalUnitType == NAL_CODED_SLICE_IDR);
  ...
  if (!IS_I_SLICE(pSliceHeader->sliceType) &&
      (!IS_P_SLICE(pSliceHeader->sliceType) || IS_IDR_NAL_UNIT(pNalUnit) || !pSeqParamSet->numRefFrames)) {
    EPRINT("slice_type");
    return (HANTRO_NOK);
  }
    ...
    if (IS_IDR_NAL_UNIT(pNalUnit) &&
        ((pSliceHeader->picOrderCntLsb > pSeqParamSet->maxPicOrderCntLsb / 2) ||
         MIN((i32)pSliceHeader->picOrderCntLsb,
             (i32)pSliceHeader->picOrderCntLsb + pSliceHeader->deltaPicOrderCntBottom) != 0)) {
      return (HANTRO_NOK);
    }
  }

  if ((pSeqParamSet->picOrderCntType == 1) && !pSeqParamSet->deltaPicOrderAlwaysZeroFlag) {
    tmp = h264bsdDecodeExpGolombSigned(pStrmData, &itmp);
    if (tmp != HANTRO_OK)
      return (tmp);
    ...
    /* check that picOrderCnt for IDR picture will be zero. See
         * DecodePicOrderCnt function to understand the logic here */
    if (IS_IDR_NAL_UNIT(pNalUnit) && MIN(pSliceHeader->deltaPicOrderCnt[0],
                                         pSliceHeader->deltaPicOrderCnt[0] + pSeqParamSet->offsetForTopToBottomField +
  ...
  if (IS_P_SLICE(pSliceHeader->sliceType)) {
    tmp = RefPicListReordering(pStrmData, &pSliceHeader->refPicListReordering, pSliceHeader->numRefIdxL0Active,
  ...
  if (pNalUnit->nalRefIdc != 0) {
    tmp =
        DecRefPicMarking(pStrmData, &pSliceHeader->decRefPicMarking, pNalUnit->nalUnitType, pSeqParamSet->numRefFrames);
    if (tmp != HANTRO_OK)
      return (tmp);
  }
  ...
  if ((pPicParamSet->numSliceGroups > 1) && (pPicParamSet->sliceGroupMapType >= 3) &&
    ...
    tmp = (picSizeInMbs + pPicParamSet->sliceGroupChangeRate - 1) / pPicParamSet->sliceGroupChangeRate;
...
u32 RefPicListReordering(strmData_t *pStrmData, refPicListReordering_t *pRefPicListReordering, u32 numRefIdxActive,
                         u32 maxPicNum) {
...
u32 DecRefPicMarking(strmData_t *pStrmData, decRefPicMarking_t *pDecRefPicMarking, nalUnitType_e nalUnitType,
                     u32 numRefFrames) {
  ...
  ASSERT(nalUnitType == NAL_CODED_SLICE_IDR || nalUnitType == NAL_CODED_SLICE || nalUnitType == NAL_SEI);
...
u32 h264bsdCheckFrameNum(strmData_t *pStrmData, u32 maxFrameNum, u32 *frameNum) {
...
u32 h264bsdCheckIdrPicId(strmData_t *pStrmData, u32 maxFrameNum, nalUnitType_e nalUnitType, u32 *idrPicId) {
...
u32 h264bsdCheckPicOrderCntLsb(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet, nalUnitType_e nalUnitType,
                               u32 *picOrderCntLsb) {
...
u32 h264bsdCheckDeltaPicOrderCntBottom(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet, nalUnitType_e nalUnitType,
...
u32 h264bsdCheckDeltaPicOrderCnt(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet, nalUnitType_e nalUnitType,
                                 u32 picOrderPresentFlag, i32 *deltaPicOrderCnt) {
...
u32 h264bsdCheckRedundantPicCnt(strmData_t *pStrmData, seqParamSet_t *pSeqParamSet, picParamSet_t *pPicParamSet,
                                nalUnitType_e nalUnitType, u32 *redundantPicCnt) {
  ...
  ASSERT(pSeqParamSet->maxFrameNum);
  ASSERT(pSeqParamSet->picOrderCntType > 0 || pSeqParamSet->maxPicOrderCntLsb);
...
u32 h264bsdCheckPriorPicsFlag(u32 *noOutputOfPriorPicsFlag, const strmData_t *pStrmData,
